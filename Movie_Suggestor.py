...
genres=df["Genre"].dropna() #removing rows with empty Genre cells
genres=genres.str.split(', ')  #splitting string into lists
genres_list=list(genres) #converting the genre series into a list

genre_choices = []  #creating an empty list to hold the genre choices
for s in genres_list:
    for i in s:
        if i not in genre_choices: #skipping duplicates
            genre_choices.append(i)
genre_choices=sorted(genre_choices)
...
def recommend(genre=None, mood=None, rating=None, n=5):
    ...
    #Shuffle the movies randomly so the results aren't always the same
    d = d.sample(frac=1).reset_index(drop=True)

    #Create an empty list to store the final movie recommendations
    out = []
...
def get_genre():
    ...
    for i, g in enumerate(genre_choices, 1): print(f"{Fore.CYAN}{i}. {g}")
    print()
    while True:
        x = input(Fore.YELLOW + "Enter genre number or name: ").strip()
        if x.isdigit() and 1 <= int(x) <= len(genre_choices): return genre_choices[int(x) - 1]
        x = x.title()
        if x in genre_choices: return x
